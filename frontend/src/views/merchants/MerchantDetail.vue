              ...
              <template #append>
                <div class="text-tertiary font-weight-bold text-body-1">
                  <template v-if="record.standard_unit_price != null">
                    <PriceWithConvert :price="record.standard_unit_price" :currency="record.currency || 'CNY'" :exchange-rate="record.exchange_rate" />{{ formatUnitSuffix(record.standard_unit_label) }}
                  </template>
                  <template v-else><PriceWithConvert :price="record.price" :currency="record.currency || 'CNY'" :exchange-rate="record.exchange_rate" /></template>
                </div>
              </template>
            </v-list-item>
          </v-list>
              ...
              <v-card elevation="0" class="list-grid-card cursor-pointer h-100" @click="goToProduct(record.product_id)">
                <v-card-text>
                  ...
                  <div class="text-h6 font-weight-bold text-tertiary mb-1">
                    <template v-if="record.standard_unit_price != null">
                      <PriceWithConvert :price="record.standard_unit_price" :currency="record.currency || 'CNY'" :exchange-rate="record.exchange_rate" /><span class="text-caption font-weight-regular text-medium-emphasis">{{ formatUnitSuffix(record.standard_unit_label) }}</span>
                    </template>
                    <template v-else><PriceWithConvert :price="record.price" :currency="record.currency || 'CNY'" :exchange-rate="record.exchange_rate" /></template>
                  </div>
