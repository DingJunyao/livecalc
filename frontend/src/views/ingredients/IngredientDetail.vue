      ...
      <v-card elevation="0" class="grid-item item-price-records">
        ...
        <v-list v-else-if="priceRecords.length > 0" lines="three">
          <v-list-item v-for="record in priceRecords" :key="record.id">
            ...
            <v-list-item-subtitle>
              <div><PriceWithConvert :price="record.price" :currency="record.currency || 'CNY'" :exchange-rate="record.exchange_rate" /> / {{ formatQuantity(record.original_quantity, localeStore.effectiveFormatLocale) }} {{ record.original_unit }}
...
const unitOptions = computed(() =>
  LOCAL_UNIT_VALUES.map((value) => ({
    title: t(LOCAL_UNIT_TRANSLATION_KEYS[value]),
    value,
  })),
)
