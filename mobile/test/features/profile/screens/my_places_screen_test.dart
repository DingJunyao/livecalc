...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockRepo.createPlace(any()))
        .thenAnswer((inv) async => UserPlace(
              id: 99,
  ...
  Future<void> pumpScreen(WidgetTester tester) async {
    ...
    await tester.pumpWidget(ProviderScope(
      overrides: [
        placeListProvider
            .overrideWith((ref) => PlaceListNotifier(mockRepo)),
        mapConfigProvider
            .overrideWith((ref) => MapConfigNotifier(merchantRepo)),
      ],
      child: MaterialApp(
        home: MyPlacesScreen(mapTileProvider: _MemoryTileProvider()),
      ),
    ));
  ...
  testWidgets('列表渲染：类型标签、星标只在默认地点、work 图标', (tester) async {
    ...
    // 元信息行：类型 · 视野 · 坐标
    expect(find.textContaining('家 · 视野 5 km · 31.2304, 121.4737'),
        findsOneWidget);
